...
/**
 * Vefifies tx entry is in Mht of tx previous to refTx. More strightforward
 * would be tx is in refTx Mht, but it also is valid proof.
 */
export function verifyTxEntryInRefTx(props: {
    entry: Entry__Output,
    ...
    const entryHash = hash.data(toBinary.fromEntry__Output_v1(props.entry))
    const entryHashLeaf = hash.leaf(entryHash)


    // compute entries mht (from entry hash and entryInTxProof)
    const entriesMht = immu.rootFromInclusionProof({
        inclusionProofNodes:    props.entryInTxProof.terms,
        lastLeafId:             Long.fromInt(props.entryInTxProof.width, true),
        leafId:                 Long.fromInt(props.entryInTxProof.leaf + 1, true), // leaf in proof is 0 indexed!!!
        leaf:                   entryHashLeaf,
    })
